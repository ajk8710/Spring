...
public class EmployeeController {
    ...
    public String saveEmployee(@Valid Employee employee, BindingResult br, Model model) {  // jpa repository automatically checks if record already exists
        ...
        if (!br.hasErrors()) {
            ...
            return "redirect:employeeForm";  // calls @RequestMapping("employeeForm") - employeeForm method
        }
        modelData(model);
    ...
    public String updateEmployee(Employee employee, Model model) {  // user object with userid and no other field passed in through route param
        ...
        employee = employeeService.findById(employee.getEmpId());  // search for the userid in db
        
        // with path="fieldName" (instead of name="fieldName") on jsp file, it's can be wired without value="${employee.fieldName}"
        System.out.println("Updating: " + employee);
        model.addAttribute("employee", employee);
        
        // below way needs value="${retrievedEmployee.fieldName}" on jsp file
