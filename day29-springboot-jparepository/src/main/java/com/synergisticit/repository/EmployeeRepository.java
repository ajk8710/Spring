...
public interface EmployeeRepository extends JpaRepository<Employee, Integer> {// <class name, type of a primary key>
    ...
    @Query("select e from Employee e where e.name like %?1%")  // HQL, Hibernate Query Language, ?1 means 1st argument
...
//    (@Param("name") String name, @Param("designation") String designation, @Param("salary") int salary);
    ...
    List<String> findAllOrderByName();  // List<String> findByOrderByName();
   
    List<Employee> findAll(Sort sort);  // using nomenclature
    
    List<Employee> findByOrderBySalaryAsc();  // using nomenclature
    List<Employee> findByOrderBySalaryDesc();
    
    @Query("select e from Employee e where e.salary>=?1 and e.salary<=?2")  // HQL, 1st and 2nd arguments
    List<Employee> findBySalaryRange(int minSalary, int maxSalary);
    
    Page<Employee> findAll(Pageable pageable);  // using nomenclature
