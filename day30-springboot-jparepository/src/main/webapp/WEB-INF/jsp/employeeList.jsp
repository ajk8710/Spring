...
<c:if test="${not empty employeesOnPage }">
<table border="1">
...
<c:forEach items="${employeesOnPage}" var="e">
	<tr>
	   ...
	   <td> <a href="updateEmployeeOnPage?empId=${e.empId}&noOfPages=${noOfPages}&noOfPages=${noOfPages}&sortedBy=${sortedBy}">Update</a> | 
	   <a href="deleteEmployeeOnPage?empId=${e.empId}">Delete</a> </td>  <!-- empId should match name of class property -->
	</tr>
</c:forEach>
...
<p>
<c:set var="noOfPages" value="${totalPages}"></c:set>  <!-- Sets properties -->
<c:set var="sortKey" value="${sortedBy}"></c:set>
<c:set var="pageSize" value="${pageSize}"></c:set>

<!-- any java code can go inside below tag -->
<%
...
out.println("<a href=\"pagedEmployee?pageNo=" + i 
    + "&pageSize=" + pageContext.getAttribute("pageSize")
    + "&sortedBy=" + pageContext.getAttribute("sortKey") + "\">"
    + i
    + "</a>");
%>
<p>${employeesOnPage}
