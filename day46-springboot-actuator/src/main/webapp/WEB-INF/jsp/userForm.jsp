...
<div align="center">
<h1>User Form</h1>
<img src="images/mango.webp" alt="Mango" style="height:3m; width:3cm;"/><br>
    ...
    <c:forEach items="${ListofAllRoles}" var="r">
        <c:choose>
            <c:when test="${selectedRoles.contains(r)}">
                <f:checkbox path="roles" value="${r.roleId}" label="${r.name}" checked="true" class="form-check-input"/>
            </c:when>
            <c:otherwise>
                <f:checkbox path="roles" value="${r.roleId}" label="${r.name}" class="form-check-input"/>
            </c:otherwise>
        </c:choose>
    </c:forEach>
...
<c:if test="${not empty users}">
    <table border="1" class="table table-striped-columns table-striped table-success">
